...
// working without subtitles block
// import { useEffect, useRef, useState } from "react";
// import Hls from "hls.js";
// import { Loader2, AlertCircle } from "lucide-react";
// import api from "@/axiosApi/axios"; // Your custom Axios instance with credentials enabled
...
//         setError(null);
//       //  const response = await api.get(`/files/${videoId}/playback`);
//         const [playbackResponse, fileResponse] = await Promise.all([
//           api.get(`/files/${videoId}/playback`),
//           api.get(`/files/${videoId}`)
//         ]);
//      //   setStreamData(response.data);
//         setStreamData(playbackResponse.data);
//         setFileData(fileResponse.data?.file || null);
//       } catch (err) {
//         console.error("HLS Token Initialization Error:", err);
//         setError(err.response?.data?.message || "Failed to fetch playback credentials.");
//       } finally {
...
export default function VideoPlayer({ videoId }) {
  ...
  // 🎯 Helper utility to build secure tokenized subtitle URLs on mount/render
  const getSecureSubtitleUrl = (trackKey) => {
    if (!streamData?.playbackUrl) return "";
    const urlObj = new URL(streamData.playbackUrl);
    const token = urlObj.searchParams.get("token");
    const fileName = trackKey.split("/").pop(); // Extracts 'subtitles.vtt' cleanly
    return `${urlObj.origin}/stream/assets/${videoId}/${fileName}${token ? `?token=${token}` : ""}`;
  };
  ...
  return (
    <video
      ...
      className="w-full h-full object-contain bg-black shadow-inner"
    >
      {/* 🔮 Map your native VTT keys from MongoDB directly over the video node element context */}
      {Array.isArray(fileData?.subtitles) &&
        fileData.subtitles.map((track, index) => (
          <track
            key={track.key}
            kind="subtitles"
            src={getSecureSubtitleUrl(track.key)}
            srcLang={track.lang || "en"}
            label={track.label || "English AI"}
            default={index === 0}
          />
        ))}
    </video>
